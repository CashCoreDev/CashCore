...
#if defined(NDEBUG)
# error "CashCore cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "CashCore Signed Message:\n";
...
CAmount GetProofOfWorkSubsidy()
{
    ...
    if (nBlockHeight <= 465000) {
    return 15 * COIN;
    }

    if (nBlockHeight <= 10000000) {
    return 2 * COIN;
    }
}
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // CashCore also requires DER encoding of pubkeys
    flags |= SCRIPT_VERIFY_DERKEY;
    // CashCore also requires low S in sigs
